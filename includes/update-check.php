...
class PluginUpdateChecker_2_1 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'Перевірити оновлення вручну');
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = 'Плагін nova podhta ttn останньої версії.';
			} else if ( $status == 'update_available' ) {
				$message = 'Доступні оновлення плагінів.';
			} else {
				$message = sprintf('Невідомий статус плагіну "%s"', htmlentities($status));
			}
			printf(
	...
	protected function isMuPlugin() {
		...
		if ( $cachedResult === null ) {
			...
			//echo '<script>console.log("'.$cachedResult.'");</script>';
		}
 ...
 * versions by calling {@link Checker::addVersion()}.
 *
 ...
 * want to instantiate one of them anyway, you can use {@link Checker::getLatestClassVersion()}
...
Checker::addVersion('PluginUpdateChecker', 'PluginUpdateChecker_2_1', '2.1');
Checker::addVersion('PluginUpdate', 'PluginUpdate_2_1', '2.1');
Checker::addVersion('PluginInfo', 'PluginInfo_2_1', '2.1');
